...
Using the bookCollection array, write a function that estimates reading effort for each book.
Reading effort is calculated as:
effort = pages + (chapters * 5) + (keywords.length * 3)
The function must:
1. Classify each book as:
"Low effort" → effort < 500
"Medium effort" → 500 ≤ effort < 900
"High effort" → effort ≥ 900
2. Return an object like:
{
  low: X,
  medium: Y,
  high: Z
}
Where the values represent how many books fall into each category.
Rules:
All books must be classified.
Do not store per-book results, only counters.
Hint: Weighted formulas, Threshold logic,Multi-branch classification


*/


function efforBooks(bookCollection){
    ...
    for(let i = 0; i < bookCollection.length; i++){
        ...
        let effor = (bookCollection[i].pages + (bookCollection[i].chapters * 5) + (bookCollection[i].keywords.length *3));

        if(effor < 500){
            bajo++;
        }else if(effor >= 500 || effor < 900){
            medio++;
        }else if(effor > 900){
            alto++;
        }
...
console.log(efforBooks(bookCollection));