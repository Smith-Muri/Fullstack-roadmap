...
function genere(bookCollection) {
    const total = {};

    for (let libro of bookCollection) {
        for (let genre of libro.genre) {
            if (!total[genre]) {
                total[genre] = 0;
            }
            total[genre] += libro.pages;
        }
    }

   
    for (let genre in total) {
        console.log(`${genre}: ${total[genre]} paginas`);
    }

    return total;
}
genere(bookCollection);
