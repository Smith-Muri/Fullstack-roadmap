...
/**
Using the bookCollection array, write a function that determines which book is the most complex, using this rule:
A book’s complexity score is calculated as:
complexity = (pages / chapters) + number of keywords
Your function must:
1. Calculate the complexity score for each book.
2. Find the book with the highest complexity score.
...
Rules:
If chapters is 0, ignore that book.
Do not store all scores in an array; track only the current maximum.
Hint: Derived metrics, Comparisons, Conditional exclusion
 */


function complejidadLibros(bookCollection){

    let maxilibro = 0;
    let maxPuntaje = 0;

    for(let i = 0; i < bookCollection.length; i++){
        const libro = bookCollection[i];

        if(libro.chapters === 0){
            continue; 
        }
        const complexityScore = (libro.pages / libro.chapters) + libro.keywords.length;

        if(complexityScore > maxPuntaje){
            maxPuntaje = complexityScore;
            maxilibro = {
                title: libro.title,
                complexityScore: parseFloat(maxPuntaje.toFixed(2))
            };
        }
    }
    return maxilibro;
...
console.log(complejidadLibros(bookCollection));