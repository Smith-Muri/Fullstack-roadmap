...
function libroFama(bookCollection) {
    let sumaG = 0;
    let contadorG = 0;

    for (let lirbo of bookCollection) {
        let sum = 0;
        let count = 0;

        for (let rating of lirbo.ratings) {
            sum += rating.score;
            count++;
            sumaG += rating.score;
            contadorG++;
        ...
        console.log(`${lirbo.title} - fama: ${average}`);
    }

    const global = (sumaG / contadorG).toFixed(2);
    console.log(`Global raiting: ${global}`);
}
libroFama(bookCollection);
