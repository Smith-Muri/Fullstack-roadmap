...
function getBooksByAvailability(booleano){

    let arrayFinal = [];
    for(let i = 0; i < bookCollection.length; i++){
        if(bookCollection[i].isAvailable === booleano){
...
function getBooksForYearsRange(year1, year2){
    let arrayFinal = [];
    for(let i = 0; i < bookCollection.length; i++){
        if((bookCollection[i].publicationYear >= year1) && (bookCollection[i].publicationYear <= year2)){
            arrayFinal.push(bookCollection[i]);
        }
...
console.log(getBooksForYearsRange(1950,1998));
